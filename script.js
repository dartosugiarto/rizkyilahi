/**
 * @file script.js
 * @description Main script for the Rizky Ilahi — Toko Bahan Bangunan & Interior (SPA).
 * @version 4.2.0 (Retail remodel)
 */
...
(function () {
  ...
  const config = {
    // Spreadsheet & nama sheet mengikuti struktur awal
    sheetId: '1B0XPR4uSvRzy9LfzWDjNjwAyMZVtJs6_Kk_r2fh7dTw',
    sheets: {
      katalog: { name: 'Sheet3' },        // daftar produk (pair kolom: Nama | Harga per kategori)
      preorder: { name1: 'Sheet1', name2: 'Sheet2' }, // tracking
      accounts: { name: 'Sheet5' },       // dipakai untuk "Interior & Jasa"
    },

    // Kontak dan preferensi checkout
    waNumber: '6285877001999',
    waGreeting: '*Detail pesanan bahan bangunan:*',
    paymentOptions: [
      { id: 'transfer_bank', name: 'Transfer Bank (BCA/BSI)', feeType: 'fixed', value: 0 },
      { id: 'qris', name: 'QRIS', feeType: 'percentage', value: 0.007 },
      { id: 'cod', name: 'COD (Area Tertentu)', feeType: 'fixed', value: 0 },
    ],
  };

  // ====== State & Element refs (sama seperti versi awal) ======
  let allCatalogData = [];
  ...
  function initializeApp() {
    ...
    elements.home.customSelect.btn.addEventListener('click', (e) => { e.stopPropagation(); toggleCustomSelect(elements.home.customSelect.wrapper); });
    elements.preorder.customSelect.btn.addEventListener('click', (e) => { e.stopPropagation(); toggleCustomSelect(elements.preorder.customSelect.wrapper); });
    elements.preorder.customStatusSelect.btn.addEventListener('click', (e) => { e.stopPropagation(); toggleCustomSelect(elements.preorder.customStatusSelect.wrapper); });
    ...
    document.addEventListener('click', () => {
      ...
      if (elements.preorder.customStatusSelect.wrapper) toggleCustomSelect(elements.preorder.customStatusSelect.wrapper, false);
    ...
    initTheme();
    loadCatalog(); // memuat data produk dari Sheet3
  }
  ...
  function setupKeyboardNavForSelect(wrapper) {
    ...
    wrapper.addEventListener('keydown', e => {
      ...
      if (isOpen && (e.key === 'ArrowDown' || e.key === 'ArrowUp')) {
        e.preventDefault();
        focusIndex = e.key === 'ArrowDown' ? Math.min(focusIndex + 1, options.length - 1) : Math.max(focusIndex - 1, 0);
        options[focusIndex]?.focus();
      }
  ...
  function showSkeleton(container, template, count = 6) {
    ...
    for (let i=0;i<count;i++) f.appendChild(template.content.cloneNode(true));
  ...
  function initTheme() { const saved = localStorage.getItem('theme'); const prefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches; const current = saved || (prefersDark ? 'dark' : 'light'); applyTheme(current); }
  function toggleTheme() { const t = document.body.classList.contains('dark-mode') ? 'light' : 'dark'; localStorage.setItem('theme', t); applyTheme(t); }
  ...
  function buildHomeCategorySelect(catalogData) {
    ...
    if (categories.length > 0) { state.home.activeCategory = categories[0].key; value.textContent = categories[0].label; }
  ...
  function renderList(container, countInfoEl, items, emptyText) {
    container.innerHTML = '';
    if (items.length === 0) {
      container.innerHTML = `<div class="empty"><div class="empty-content">
        <svg xmlns="http://www.w3.org/2000/svg" class="empty-icon" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" /></svg>
        <p>${emptyText}</p></div></div>`;
  ...
  function renderHomeList() {
    ...
    const items = allCatalogData.filter(x =>
      x.catKey === activeCategory && (query === '' || x.title.toLowerCase().includes(query) || String(x.price).includes(query)));
    renderList(elements.home.listContainer, elements.home.countInfo, items, 'Tidak ada produk ditemukan.');
  }
  ...
  async function loadCatalog() {
    ...
    try {
      ...
      if (allCatalogData.length === 0) throw new Error('Data kosong atau format salah.');
  ...
  } // (sesuai pola asli untuk memuat katalog)  [oai_citation:4‡script.js](file-service://file-UVbNGWzv3C2pLS559TcTL4)
  ...
  function updateWaLink(option, fee, total) {
    ...
    const text = [
      ...
      `› Biaya Tambahan: ${formatToIdr(fee)}`,
  ...
  function openPaymentModal(item) {
    ...
    modalFocusTrap.listener = function (e) {
      if (e.key !== 'Tab') return;
      if (e.shiftKey) {
        if (document.activeElement === modalFocusTrap.firstEl) { modalFocusTrap.lastEl.focus(); e.preventDefault(); }
      } else {
        if (document.activeElement === modalFocusTrap.lastEl) { modalFocusTrap.firstEl.focus(); e.preventDefault(); }
      }
    };
  ...
  function renderPreorderCards() {
    ...
    if (pageData.length === 0) {
      listContainer.innerHTML = `<div class="empty"><div class="empty-content">
        <svg xmlns="http://www.w3.org/2000/svg" class="empty-icon" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" /></svg>
        <p>Tidak Ada Hasil Ditemukan</p></div></div>`;
  ...
  // ==== INTERIOR & JASA (menggunakan Sheet5) ====
  ...
  function populateAccountSelect() {
    ...
    options.innerHTML = '';
    if (state.accounts.data.length === 0) { value.textContent = 'Data belum ada'; empty.style.display = 'block'; return; }
    value.textContent = 'Pilih Paket/Jasa';
    state.accounts.data.forEach((acc, index) => {
      const el = document.createElement('div');
      el.className = 'custom-select-option';
      el.textContent = acc.title;
      el.dataset.value = index;
      el.setAttribute('role', 'option'); el.setAttribute('tabindex', '-1');
      el.addEventListener('click', () => {
  ...
  function renderAccount(index) {
    ...
    if (account.images && account.images.length > 0) {
      account.images.forEach((src, i) => {
        track.insertAdjacentHTML('beforeend', `<div class="carousel-slide"><img src="${src}" alt="Gambar ${i+1} untuk ${account.title}" decoding="async" loading="lazy"></div>`);
    ...
    indicators.querySelectorAll('.indicator-dot').forEach(dot => {
      dot.addEventListener('click', e => { e.stopPropagation(); state.accounts.currentIndex = parseInt(e.target.dataset.index); updateCarousel(); });
    });
  ...
  function initializeCarousel() {
    ...
    track.addEventListener('touchend', e => { e.stopPropagation(); const x = e.changedTouches[0].screenX; if (x < touchStartX - 50) nextBtn.click(); if (x > touchStartX + 50) prevBtn.click(); }, { passive: true });
  }
  ...
  async function initializeAccounts() {
    ...
    try {
      ...
      error.textContent = 'Gagal memuat data jasa. Coba lagi nanti.';
    ...
    buyBtn.addEventListener('click', e => { e.stopPropagation(); if (state.accounts.currentAccount) { openPaymentModal({ title: state.accounts.currentAccount.title, price: state.accounts.currentAccount.price, catLabel: 'Jasa & Interior' }); } });
    offerBtn.addEventListener('click', e => { e.stopPropagation(); if (state.accounts.currentAccount) { const text = `Halo, saya tertarik dengan Jasa/Proyek: ${state.accounts.currentAccount.title}`; window.open(`https://wa.me/${config.waNumber}?text=${encodeURIComponent(text)}`, '_blank', 'noopener'); } });

    initializeCarousel();
    state.accounts.initialized = true;
  }

  // ==== PERPUSTAKAAN (Brosur & Panduan) ====
  async function initializeLibrary() {
    const container = getElement('libraryGridContainer');
    const errorEl = getElement('libraryError');

    container.innerHTML = '<p>Memuat brosur...</p>';
    ...
    try {
      ...
      const books = rows
        ...
        .map(row => ({ title: row[0], coverUrl: row[1], bookUrl: row[2] }));
      renderLibraryGrid(books);

    } catch (err) {
      console.error('Failed to load library:', err);
      container.innerHTML = '';
      errorEl.textContent = 'Gagal memuat brosur. Coba lagi nanti.';
  ...
  function renderLibraryGrid(books) {
    ...
    if (!books || books.length === 0) {
      container.innerHTML = '<div class="empty">Belum ada brosur ditambahkan.</div>';
      return;
    }
    container.innerHTML = '';
    const f = document.createDocumentFragment();
  ...
  // Integrasi setMode agar memuat perpustakaan saat dibuka
  ...
  function pp_makeNodes(list) {
    ...
    list.forEach(({ name, url }) => {
      ...
      li.innerHTML =
        '<figure class="testi-fig"><img src="' + url + '" alt="Testimoni ' + name.replace(/"/g,'&quot;') + '" decoding="async" loading="lazy" referrerpolicy="no-referrer"></figure>' +
  ...
  async function loadTestimonials() {
    ...
    try {
      ...
      const items = data.filter(r => r && r[0] && r[1]).map(r => ({ name: String(r[0]).trim(), url: String(r[1]).trim() }));
      if (!items.length) { section.style.display = 'none'; return; }

      track.innerHTML = '';
      track.appendChild(pp_makeNodes(items));
      track.appendChild(pp_makeNodes(items)); // duplikasi untuk loop
      ...
      function step(ts){
        ...
        if (!dragging && !pausedByHover && !reduceMotion) { pos -= speed * dt; applyTransform(); }
      ...
      function onPointerDown(e){ dragging = true; track.classList.add('dragging'); startX = (e.touches ? e.touches[0].clientX : e.clientX); startPos = pos; marquee.setPointerCapture && marquee.setPointerCapture(e.pointerId || 1); e.preventDefault(); }
      function onPointerMove(e){ if (!dragging) return; const x = (e.touches ? e.touches[0].clientX : e.clientX); pos = startPos + (x - startX); applyTransform(); }
      function onPointerUp(e){ dragging = false; track.classList.remove('dragging'); marquee.releasePointerCapture && marquee.releasePointerCapture(e.pointerId || 1); }
  ...
  // aksesibilitas kecil: ESC menutup modal/menu
